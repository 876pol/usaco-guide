    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < q; i++) {
            ...
            if (op == 1) {
                ...
                io.println(segmentTree.query(1, 0, n - 1, a - 1, b - 1).pref);
            }
        }
        io.flush();
    }
...
class SegmentTree {
    class Node {
        ...
        public Node(long sum, long pref) {
            this.sum = sum;
            this.pref = pref;
        }
    }
    ...
    private void build(int v, int tl, int tr, long[] a) {
        if (tl == tr) {
            tree[v].sum = a[tl];
            tree[v].pref = Math.max(0, a[tl]);
        } else {
            int tm = (tl + tr) / 2;
            build(v + 1, tl, tm, a);
            build(v + 2 * (tm - tl + 1), tm + 1, tr, a);
            tree[v].sum = tree[v + 1].sum + tree[v + 2 * (tm - tl + 1)].sum;
            tree[v].pref = Math.max(tree[v + 1].pref, tree[v + 1].sum + tree[v + 2 * (tm - tl + 1)].pref);
        }
    }
    ...
    public Node query(int v, int tl, int tr, int l, int r) {
        ...
        int tm = (tl + tr) / 2;
        Node n1 = query(v + 1, tl, tm, l, r);
        Node n2 = query(v + 2 * (tm - tl + 1), tm + 1, tr, l, r);
        return new Node(n1.sum + n2.sum, Math.max(n1.pref, n1.sum + n2.pref));
    }

    public void update(int v, int tl, int tr, int ind, int val) {
        if (tl == tr) {
            tree[v].sum = val;
            tree[v].pref = Math.max(0, val);
        } else {
            int tm = (tl + tr) / 2;
            if (ind <= tm) {
                update(v + 1, tl, tm, ind, val);
            } else {
                update(v + 2 * (tm - tl + 1), tm + 1, tr, ind, val);
            }
            tree[v].sum = tree[v + 1].sum + tree[v + 2 * (tm - tl + 1)].sum;
            tree[v].pref = Math.max(tree[v + 1].pref, tree[v + 1].sum + tree[v + 2 * (tm - tl + 1)].pref);
        }
